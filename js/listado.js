...
var eliminarDuplicados = array => {
    return array.sort().reduce(function (a, b) { if (a.indexOf(b) < 0) a.push(b); return a; }, []);
    }
...
Listado.prototype.obtenerUbicaciones = function() {
    const arrayCiudades = this.restaurantes.map(elemento => elemento.ubicacion);
    return eliminarDuplicados(arrayCiudades);
}
...
Listado.prototype.obtenerRubros = function() {
    const arrayRubros = this.restaurantes.map(elemento => elemento.rubro);
    return eliminarDuplicados(arrayRubros);
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var arregloHorario= this.restaurantes.map(function(restaurantes){
        return restaurantes.horarios;
    })

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horario = [];
    arregloHorario.forEach(function(a) {
    ...
    return eliminarDuplicados(horario).sort();
}
